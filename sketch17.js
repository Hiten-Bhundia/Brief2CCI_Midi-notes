// ------------------------------------------------------------
// 6-Dataset Harmony Visualiser + Stacked Timelines (Cleaned UI)
// ------------------------------------------------------------
...
let timelineY = { d1:150, d2:190, d3:230, j1:270, j2:310, j3:350 };
...
function draw(){
  ...
  // Notes (moved right)
  fill(harmonyName==="Not in Harmony"?"rgb(243, 226, 198)":"rgb(100, 160, 2)");
  textAlign(LEFT);
  textSize(20);
  ...
  // Draw all timelines stacked (shortened & middle-left)
  const timelineWidth=300;
  drawTimeline(timelineD1,50,timelineY.d1,timelineWidth,12,activeDataset==="d1"?i:null);
  drawTimeline(timelineD2,50,timelineY.d2,timelineWidth,12,activeDataset==="d2"?i:null);
  drawTimeline(timelineD3,50,timelineY.d3,timelineWidth,12,activeDataset==="d3"?i:null);
  drawTimeline(timelineJ1,50,timelineY.j1,timelineWidth,12,activeDataset==="j1"?i:null);
  drawTimeline(timelineJ2,50,timelineY.j2,timelineWidth,12,activeDataset==="j2"?i:null);
  drawTimeline(timelineJ3,50,timelineY.j3,timelineWidth,12,activeDataset==="j3"?i:null);
}
...
function drawHarmonyPie(x,y,radius,stats){
  if(!stats) return;
  let start=0;
  fill("rgb(100, 160, 2)"); let inArc=stats.inHarmony*TWO_PI; arc(x,y,radius*2,radius*2,start,start+inArc); start+=inArc;
  fill("rgb(243, 226, 198)"); let notArc=stats.notInHarmony*TWO_PI; arc(x,y,radius*2,radius*2,start,start+notArc);
  fill("rgb(100, 160, 2)"); textSize(14); textAlign(CENTER); text(Math.round(stats.inHarmony*100)+"% In Harmony",x,y+60);
  fill("rgb(243, 226, 198)"); text(Math.round(stats.notInHarmony*100)+"% Not in Harmony",x,y-60);
}
...
function drawTimeline(arr,x,y,w,h,index){
  ...
  noStroke();
  for(let j=0;j<arr.length;j++){ fill(arr[j]===1?"rgb(100, 160, 2)":"rgb(243, 226, 198)"); rect(x+j*step,y,step,h); }
