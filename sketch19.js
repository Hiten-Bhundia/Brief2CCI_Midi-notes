...
// Buttons configurable like timelines
// ------------------------------------------------------------
...
const BUTTON_X = TIMELINE_X - 70;  
const BUTTON_Y_START = TIMELINE_Y_START + 45;  
...
function draw(){
  ...
  text("P: "+midiToNoteName(midi1), 1250, 400);
  text("G: "+midiToNoteName(midi2), 1250, 450);
  text("R: "+midiToNoteName(midi3), 1250, 500);
  textSize(26); text("Chord Type: "+harmonyName, 1250, 550);
...
function drawHarmonyPie(x,y,radius,stats){
  ...
  fill(COLOR_GREEN); textSize(14); textAlign(CENTER); text(Math.round(stats.inHarmony*100)+"% Chords detected",x,y+70);
  fill(COLOR_YELLOW); text(Math.round(stats.notInHarmony*100)+"% No Chords detected",x,y-60);
}
