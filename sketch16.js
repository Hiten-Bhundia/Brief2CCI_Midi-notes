// ------------------------------------------------------------
// 6-Dataset Harmony Visualiser + Stacked Timelines
// ------------------------------------------------------------
...
function draw(){
  ...
  fill(harmonyName==="Not in Harmony"?"red":"green"); textAlign(CENTER); textSize(20);
  text("Loop:"+loopCount,width/2,50); text("Frame:"+i,width/2,80);
  text("Timestamp:",width/2,110); text(frame.timestamp,width/2,135);

  // Pie chart
  let stats=currentData==="d1"?statsD1:currentData==="d2"?statsD2:currentData==="d3"?statsD3:
            currentData==="j1"?statsJ1:currentData==="j2"?statsJ2:currentData==="j3"?statsJ3:null;
  ...
  drawTimeline(timelineD3,50,timelineY.d3,width-100,20,activeDataset==="d3"?i:null);
  ...
  drawTimeline(timelineJ3,50,timelineY.j3,width-100,20,activeDataset==="j3"?i:null);
}
...
function drawTimeline(arr,x,y,w,h,index){
  ...
  noStroke();
  for(let j=0;j<arr.length;j++){ fill(arr[j]===1?"green":"red"); rect(x+j*step,y,step,h); }
  if(index!==null){ stroke(0); strokeWeight(2); let markerX=x+index*step; line(markerX,y-5,markerX,y+h+5); }
}
